...
function cleanupPinnedRepos(data: PinnedReposResponse) {
  return data.organization.pinnedItems.edges.map(({ node }) => {
    ...
    const sortedTopics = topicEntries.sort((a, b) => a.topic.stargazerCount - b.topic.stargazerCount)

    const { topics } = sortedTopics.reduce((acc, entry) => {
    ...
    return {
      topics: topics.map(({ topic: { name, stargazerCount }, url }) => ({ name, url, stars: stargazerCount })),
      ...node,
    }
  }
  )
